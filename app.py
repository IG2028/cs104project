    ...
    try:
        with open(path_to_read, 'r') as file:
            for idx, line in enumerate(file):
                ...
                row = {columns[i]: row_list[i] for i in range(len(columns))}
                log.append(row)
...
def plot_events_vs_time(timestamps, freq, filename):
    ...
    uniform_time = np.arange(start_time, end_time + 1, 1)
    ...
    plt.tight_layout()
    plt.savefig(f'static/images/{filename}_freq_with_time.png')
    plt.close()
...
def plot_level_distribution(labels, values, filename):
    ...
    plt.tight_layout()
    plt.savefig(f'static/images/{filename}_level_count.png')
    plt.close()

# Bar chart of event codes
def plot_event_code_distribution(labels, values, filename):
    ...
    plt.tight_layout()
    plt.savefig(f'static/images/{filename}_event_freq.png')
    plt.close()
